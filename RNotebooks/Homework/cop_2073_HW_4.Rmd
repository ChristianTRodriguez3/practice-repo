...
makes <- vehicles %>%
  group_by(make) %>%
  summarise(count = n_distinct(make)) %>%
  select(make)
makes
```
...
flights %>%
  ...
  summarise(ave = mean(air_time, na.rm = TRUE), minimum = min(air_time, na.rm = TRUE), maximum = max(air_time, na.rm = TRUE))
```
...
```{r}
flights %>%
  filter(dest == "ORL" | dest == "TPA" | dest == "MIA", sched_dep_time > 2000) %>%
  arrange(dep_time)
```
