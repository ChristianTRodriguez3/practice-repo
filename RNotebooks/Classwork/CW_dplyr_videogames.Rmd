...
```{r}
vg_sales_and_check <- mutate(vg_sales, Global_Sales_check = NA_Sales + EU_Sales + JP_Sales + Other_Sales)
filter(vg_sales_and_check, Global_Sales != Global_Sales_check)
```
...
```{r}
arrange(vg_sales, desc(Global_Sales), desc(Year))
```
